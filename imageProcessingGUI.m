function create_gui()
    ...
    mainLayout.RowHeight = {300, 300};        % 第一行300px，第二行300px
    % 图像显示区域
    ...
    % 按钮布局
    buttonLayout = uigridlayout(buttonPanel, [7, 4]); % 7行四列的布局
    ...
    logButton = uibutton(buttonLayout, 'Text', '对数增强', 'ButtonPushedFcn', @(src, event) logButton_Callback());
    expButton = uibutton(buttonLayout, 'Text', '指数增强', 'ButtonPushedFcn', @(src, event) expButton_Callback());
    ...
    function logButton_Callback(hObject, eventdata)
        if isfield(handles, 'img')
            ...
            logImg = 46*log(double(img) +1);
            logImg = mat2gray(logImg);  % 将对数增强图像归一化到 [0, 1]
            axes(handles.imageAxes);
            imshow(logImg);
            title('对数增强');
    ...
    function expButton_Callback(hObject, eventdata)
        if isfield(handles, 'img')
            ...
            %expImg = 185*exp(0.325*(double(img)-225)/30)+1;
            %expImg = min(max(expImg, 0), 1);  % 保证值在 [0, 1] 范围内
            % 将结果转换回 0 到 255 范围，并转为 uint8 类型
            %expImg = uint8(expImg * 255);
            axes(handles.imageAxes);
            imshow(expImg);
            title('指数增强');
 ...
 % 回调函数：加噪声和滤波
    ...
    if isfield(handles, 'img')    
        ...
        stats = regionprops(erodedImg, 'BoundingBox', 'Area', 'Centroid'); %连通域分析，提取目标
